...
| contactstaff.php | 14 | ✅ | P3 | 联系管理组。`ContactStaffController::web()` 迁移遗留 `public/contactstaff.php`（联系表单页，提交走 `takecontact.php`，渲染时校验收件人），渲染 Blade `contactstaff`，路由 `/contactstaff.php`（GET，auth.nexus），遗留文件已删；`ContactStaffPageTest` 覆盖 |
| takecontact.php | 47 | ✅ | P3 | 联系提交。`ContactStaffController::webTakeContact()` 迁移遗留 `public/takecontact.php`（POST `body`+`subject`+`returnto`、非 MODERATOR 一分钟限流、`StaffMessage::add()` + 更新 `users.last_staffmsg`、`clear_staff_message_cache()`、有 returnto 重定向否则成功页），路由 `/takecontact.php`（GET+POST，auth.nexus），遗留文件已删；`TakeContactPageTest` 覆盖 |
...
| settings.php | 970 | ✅ | P1 | `SettingsController::web()` (SYSOP 站点设置，含 main/basic/smtp/security/authority/tweak/bonus/account 等子表单 + 保存），路由 `/settings.php`，遗留文件已删；`SettingsPageTest` 覆盖 |
...
| mysql_stats.php | 370 | ✅ | P3 | MySQL 状态页（SYSOP）。`MysqlStatsController::web()` 迁移遗留 `public/mysql_stats.php`（`DB::select('SHOW STATUS')` 读取状态变量：Uptime/启动时间、流量（收/发/总 + 每小时速率）、连接计数、按语句类型分组的 Query Statistics、其余状态变量三列分栏），渲染 Blade `mysql_stats`，路由 `/mysql_stats.php`（GET，auth.nexus），遗留文件已删；`MysqlStatsPageTest` 覆盖 |
