...
{
    /**
     * Website settings page (SYSOP) — replaces legacy public/settings.php.
     *
     * GET /settings.php renders the settings menu (action=showmenu);
     * POSTing a sub-action renders the matching form (basicsettings,
     * mainsettings, smtpsettings, ...) and POSTing a savesettings_* action
     * persists the form via saveSetting(). Output is captured through the
     * shared legacy helpers and wrapped in the Blade layout.
     */
...
<font color=\"white\">".$lang_settings['text_configuration_file_saving_note']."
        ...
        $pageTitle = $lang_settings['text_website_settings'];

        ob_start();

        if ($action == 'savesettings_main')	// save main
        {
            do_log(json_encode($request->all()));
            $pageTitle = $lang_settings['head_save_main_settings'];
            $validConfig = array(
            ...
            foreach($validConfig as $config) {
                $MAIN[$config] = $GLOBALS[$config] ?? null;
            }

            saveSetting('main', $MAIN);
            if (isset($GLOBALS['Cache'])) {
                $GLOBALS['Cache']->delete_value('recent_news', true);
                $GLOBALS['Cache']->delete_value('stats_users', true);
                $GLOBALS['Cache']->delete_value('stats_torrents', true);
                $GLOBALS['Cache']->delete_value('peers_count', true);
                $GLOBALS['Cache']->delete_value('site_lang_lang_list', true);
            }
        ...
        {
            $pageTitle = $lang_settings['head_save_basic_settings'];
            $validConfig = array(
            ...
            $BASIC = [];
            foreach($validConfig as $config) {
                $BASIC[$config] = $GLOBALS[$config] ?? null;
            }
        ...
        {
            $pageTitle = $lang_settings['head_save_code_settings'];
            $validConfig = array('mainversion','subversion','releasedate','website');
            GetVar($validConfig);
            $CODE = [];
            foreach($validConfig as $config) {
                $CODE[$config] = $GLOBALS[$config] ?? null;
            }
        ...
        {
            $pageTitle = $lang_settings['head_save_bonus_settings'];
            $validConfig = array(
            ...
            $BONUS = [];
            foreach($validConfig as $config) {
                $BONUS[$config] = $GLOBALS[$config] ?? null;
            }
            $BONUS['attendance_continuous'] = array();
            $attendanceContinuousDay = $request->input('attendance_continuous_day', []);
            $attendanceContinuousValue = $request->input('attendance_continuous_value', []);
            if(count($attendanceContinuousDay) == count($attendanceContinuousValue)){
                foreach($attendanceContinuousDay as $k => $day){
                    $value = (int) $attendanceContinuousValue[$k];
        ...
        {
            $pageTitle = $lang_settings['head_save_account_settings'];
            ...
            $ACCOUNT = [];
            foreach($validConfig as $config) {
                $ACCOUNT[$config] = $GLOBALS[$config] ?? null;
            }
        ...
        {
            $pageTitle = $lang_settings['head_save_torrent_settings'];
            $validConfig = array(
            ...
            $TORRENT = [];
            foreach($validConfig as $config) {
                $TORRENT[$config] = $GLOBALS[$config] ?? null;
            }
        ...
        {
            $pageTitle = $lang_settings['head_save_smtp_settings'];
            $validConfig = array('smtptype', 'emailnotify');
            GetVar($validConfig);
            if (isset($GLOBALS['smtptype']) && $GLOBALS['smtptype'] == 'advanced') {
                $validConfig = array_merge($validConfig, array('smtp_host','smtp_port','smtp_from'));
            } elseif (isset($GLOBALS['smtptype']) && $GLOBALS['smtptype'] == 'external') {
            ...
            $SMTP = [];
            foreach($validConfig as $config) {
                $SMTP[$config] = $GLOBALS[$config] ?? null;
            }
        ...
        {
            $pageTitle = $lang_settings['head_save_security_settings'];
            $validConfig = array(
            ...
            $SECURITY = [];
            foreach($validConfig as $config) {
                $SECURITY[$config] = $GLOBALS[$config] ?? null;
            }
            if (($request->input('login_secret_regenerate') ?? '') == 'yes') {
                $minute = intval($request->input('login_secret_lifetime'));
        ...
        {
            $pageTitle = $lang_settings['head_save_authority_settings'];
            $validConfig = array(
            ...
            $AUTHORITY = [];
            foreach($validConfig as $config) {
                $AUTHORITY[$config] = $GLOBALS[$config] ?? null;
                if (in_array($config, Setting::$permissionMustHaveClass)) {
                    if (!isset(User::$classes[$AUTHORITY[$config]])) {
                        ob_end_clean();
                        $content = $this->capture(function () use ($lang_settings, $AUTHORITY, $config) {
                            stderr('Error', 'Invalid user class: ' . $AUTHORITY[$config], false, false, false, false);
                        });
                        return view('settings', compact('content') + ['pageTitle' => 'Error']);
                    }
                }
            }
        ...
        {
            $pageTitle = $lang_settings['head_save_tweak_settings'];
            ...
            $TWEAK = [];
            foreach($validConfig as $config) {
                $TWEAK[$config] = $GLOBALS[$config] ?? null;
            }
        ...
        {
            $pageTitle = $lang_settings['head_save_attachment_settings'];
            ...
            $ATTACHMENT = [];
            foreach($validConfig as $config) {
                $ATTACHMENT[$config] = $GLOBALS[$config] ?? null;
            }
        ...
        {
            $pageTitle = $lang_settings['row_misc_settings'];
            $validConfig = array('donation_custom', 'protected_forum',);
            GetVar($validConfig);
            $data = [];
            if (!empty($GLOBALS['protected_forum'] ?? '') && !preg_match("/^[,\\d]*[\\d]+$/", $GLOBALS['protected_forum'])){
                ob_end_clean();
                $content = $this->capture(function () use ($lang_settings) {
                    stderr($lang_settings['std_error'], $lang_settings['forum_format_error'].'<br>'.$lang_settings['std_click']."<a class=\"altlink\" href=\"settings.php\">".$lang_settings['std_here']."</a>".$lang_settings['std_to_go_back'], false, false, false, false);
                });
                return view('settings', compact('content') + ['pageTitle' => $lang_settings['std_error']]);
            }
            foreach($validConfig as $config) {
                $data[$config] = $GLOBALS[$config] ?? null;
            }
        ...
        elseif ($action == 'tweaksettings')		// tweak settings
        {
            ...
            $pageTitle = $lang_settings['head_tweak_settings'];
            print ($notice);
            print ("<form method='post' action='".$_SERVER["SCRIPT_NAME"]."'><input type='hidden' name='action' value='savesettings_tweak' />");
            $this->yesorno($lang_settings['row_save_user_location'], 'where', $TWEAK["where"], $lang_settings['text_save_user_location_note']);
            ...
            $this->yesorno($lang_settings['row_enable_location'], 'enablelocation', $TWEAK["enablelocation"], $lang_settings['text_enable_location_note']);
            $this->yesorno($lang_settings['row_enable_tooltip'], 'enabletooltip', $TWEAK["enabletooltip"], $lang_settings['text_enable_tooltip_note']);
            ...
            tr($lang_settings['row_see_sql_debug'], "<input type='checkbox' name='enablesqldebug' value='yes'".($TWEAK['enablesqldebug'] == 'yes' ? " checked='checked'" : "")." />".$lang_settings['text_allow'].classlist('sqldebug',User::CLASS_STAFF_LEADER,$TWEAK['sqldebug'], User::CLASS_MODERATOR).$lang_settings['text_see_sql_list'].get_user_class_name(User::CLASS_SYSOP,false,true,true),1);
        ...
        elseif ($action == 'smtpsettings')	// stmp settings
        {
            ...
            $pageTitle = $lang_settings['head_smtp_settings'];
            print ($notice);
            print("<tbody>");
            print ("<form method='post' action='".$_SERVER["SCRIPT_NAME"]."'><input type='hidden' name='action' value='savesettings_smtp'>");
            $this->yesorno($lang_settings['row_enable_email_notification'], 'emailnotify', $SMTP["emailnotify"], $lang_settings['text_email_notification_note']);
            ...
            $siteEmail = get_setting_from_db('main.SITEEMAIL', '');
            if (strtoupper(substr(PHP_OS,0,3)=='WIN'))
                tr($lang_settings['row_smtp_sendmail_from'], "<input type='text' style=\"width: 300px\" name=smtp_from value='".($SMTP['smtp_from'] ? $SMTP['smtp_from'] : $siteEmail)."'> ".$lang_settings['text_smtp_sendmail_from_note'].$siteEmail, 1);
            else
        ...
        elseif ($action == 'securitysettings')	//security settings
        {
            ...
            $pageTitle = $lang_settings['head_security_settings'];
            print ($notice);
            print("<tbody>");
            ...
            $this->yesorno($lang_settings['row_use_challenge_response_authentication'], 'use_challenge_response_authentication', $SECURITY["use_challenge_response_authentication"], $lang_settings['text_use_challenge_response_authentication_note']);
        ...
        {
            $AUTHORITY = get_setting_from_db('authority');
            $pageTitle = $lang_settings['head_authority_settings'];
            print ($notice);
            $maxclass = User::CLASS_SYSOP;
            print ("<form method='post' action='".$_SERVER["SCRIPT_NAME"]."'><input type='hidden' name='action' value='savesettings_authority'>");
            tr($lang_settings['row_default_class'], $lang_settings['text_default_user_class'].classlist('defaultclass',User::CLASS_STAFF_LEADER,$AUTHORITY['defaultclass']).$lang_settings['text_default'].get_user_class_name(User::CLASS_USER,false,true,true).$lang_settings['text_default_class_note'], 1);
            tr($lang_settings['row_staff_member'], $lang_settings['text_minimum_class'].classlist('staffmem',User::CLASS_STAFF_LEADER,$AUTHORITY['staffmem']).$lang_settings['text_default'].get_user_class_name(User::CLASS_MODERATOR,false,true,true).$lang_settings['text_staff_member_note'], 1);
            tr($lang_settings['row_news_management'], $lang_settings['text_minimum_class'].classlist('newsmanage',$maxclass,$AUTHORITY['newsmanage'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_ADMINISTRATOR,false,true,true).$lang_settings['text_news_management_note'],1);
            tr($lang_settings['row_post_funbox_item'], $lang_settings['text_minimum_class'].classlist('newfunitem',$maxclass,$AUTHORITY['newfunitem'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_USER,false,true,true).$lang_settings['text_post_funbox_item_note'],1);
            tr($lang_settings['row_funbox_management'], $lang_settings['text_minimum_class']. classlist('funmanage',$maxclass,$AUTHORITY['funmanage'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_MODERATOR,false,true,true).$lang_settings['text_funbox_management_note'],1);
            tr($lang_settings['row_shoutbox_management'], $lang_settings['text_minimum_class']. classlist('sbmanage',$maxclass,$AUTHORITY['sbmanage'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_MODERATOR,false,true,true).$lang_settings['text_shoutbox_management_note'],1);
            tr($lang_settings['row_poll_management'], $lang_settings['text_minimum_class'].classlist('pollmanage',$maxclass,$AUTHORITY['pollmanage'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_ADMINISTRATOR,false,true,true).$lang_settings['text_poll_management_note'],1);
            tr($lang_settings['row_apply_for_links'], $lang_settings['text_minimum_class'].classlist('applylink',$maxclass,$AUTHORITY['applylink'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_USER,false,true,true).$lang_settings['text_apply_for_links_note'],1);
            tr($lang_settings['row_link_management'], $lang_settings['text_minimum_class'].classlist('linkmanage',$maxclass,$AUTHORITY['linkmanage'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_ADMINISTRATOR,false,true,true).$lang_settings['text_link_management_note'],1);
            tr($lang_settings['row_forum_post_management'], $lang_settings['text_minimum_class'].classlist('postmanage',$maxclass,$AUTHORITY['postmanage'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_MODERATOR,false,true,true).$lang_settings['text_forum_post_management_note'],1);
            tr($lang_settings['row_comment_management'], $lang_settings['text_minimum_class'].classlist('commanage',$maxclass,$AUTHORITY['commanage'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_MODERATOR,false,true,true).$lang_settings['text_comment_management_note'],1);
            tr($lang_settings['row_forum_management'], $lang_settings['text_minimum_class'].classlist('forummanage',$maxclass,$AUTHORITY['forummanage'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_ADMINISTRATOR,false,true,true).$lang_settings['text_forum_management_note'],1);
            tr($lang_settings['row_view_userlist'], $lang_settings['text_minimum_class'].classlist('viewuserlist',$maxclass,$AUTHORITY['viewuserlist'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_POWER_USER,false,true,true).$lang_settings['text_view_userlist_note'],1);
            tr(nexus_trans('permission.user-delete.text'), $lang_settings['text_minimum_class'].classlist('user-delete',$maxclass,$AUTHORITY['user-delete'] ?? '',0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_ADMINISTRATOR,false,true,true).nexus_trans('permission.user-delete.desc'),1);
            tr(nexus_trans('permission.user-change-class.text'), $lang_settings['text_minimum_class'].classlist('user-change-class',$maxclass,$AUTHORITY['user-change-class'] ?? '',0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_ADMINISTRATOR,false,true,true).nexus_trans('permission.user-change-class.desc'),1);

            tr($lang_settings['row_torrent_management'], $lang_settings['text_minimum_class'].classlist('torrentmanage',$maxclass,$AUTHORITY['torrentmanage'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_MODERATOR,false,true,true).$lang_settings['text_torrent_management_note'], 1);
            tr($lang_settings['row_torrent_delete'], $lang_settings['text_minimum_class'].classlist('torrent-delete',$maxclass,$AUTHORITY['torrent-delete'] ?? '',0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_ADMINISTRATOR,false,true,true).$lang_settings['text_torrent_delete_note'], 1);


            tr($lang_settings['row_torrent_sticky'], $lang_settings['text_minimum_class'].classlist('torrentsticky',$maxclass,$AUTHORITY['torrentsticky'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_ADMINISTRATOR,false,true,true).$lang_settings['text_torrent_sticky_note'],1);
            tr($lang_settings['row_torrent_on_promotion'], $lang_settings['text_minimum_class'].classlist('torrentonpromotion',$maxclass,$AUTHORITY['torrentonpromotion'] ?? '',0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_ADMINISTRATOR,false,true,true).$lang_settings['text_torrent_promotion_note'],1);
            tr($lang_settings['row_torrent_hr'], $lang_settings['text_minimum_class'].classlist('torrent_hr',$maxclass,$AUTHORITY['torrent_hr'] ?? '',0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_ADMINISTRATOR,false,true,true).$lang_settings['text_torrent_hr_note'],1);
            tr(nexus_trans('permission.torrent-set-special-tag.text'), $lang_settings['text_minimum_class'].classlist('torrent-set-special-tag',$maxclass,$AUTHORITY['torrent-set-special-tag'] ?? '',0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_ADMINISTRATOR,false,true,true).nexus_trans('permission.torrent-set-special-tag.desc'),1);
            tr(nexus_trans('permission.torrent-approval.text'), $lang_settings['text_minimum_class'].classlist('torrent-approval',$maxclass,$AUTHORITY['torrent-approval'] ?? '',0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_ADMINISTRATOR,false,true,true).nexus_trans('permission.torrent-approval.desc'),1);
            tr(nexus_trans('permission.torrent-approval-allow-automatic.text'), $lang_settings['text_minimum_class'].classlist('torrent-approval-allow-automatic',$maxclass,$AUTHORITY['torrent-approval-allow-automatic'] ?? '',0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_UPLOADER,false,true,true).nexus_trans('permission.torrent-approval-allow-automatic.desc'),1);
            tr(nexus_trans('permission.torrent-set-price.text'), $lang_settings['text_minimum_class'].classlist('torrent-set-price',$maxclass,$AUTHORITY['torrent-set-price'] ?? '',0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_UPLOADER,false,true,true).nexus_trans('permission.torrent-set-price.desc'),1);

            tr($lang_settings['row_ask_for_reseed'],  $lang_settings['text_minimum_class'].classlist('askreseed',$maxclass,$AUTHORITY['askreseed'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_POWER_USER,false,true,true).$lang_settings['text_ask_for_reseed_note'],1);
            tr($lang_settings['row_view_nfo'], $lang_settings['text_minimum_class'].classlist('viewnfo',$maxclass,$AUTHORITY['viewnfo'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_POWER_USER,false,true,true).$lang_settings['text_view_nfo_note'],1);
            tr($lang_settings['row_view_torrent_structure'], $lang_settings['text_minimum_class'].classlist('torrentstructure',$maxclass,$AUTHORITY['torrentstructure'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_ULTIMATE_USER,false,true,true).$lang_settings['text_view_torrent_structure_note'],1);
            tr($lang_settings['row_send_invite'], $lang_settings['text_minimum_class'].classlist('sendinvite',$maxclass,$AUTHORITY['sendinvite'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_POWER_USER,false,true,true).$lang_settings['text_send_invite_note'],1);
            tr($lang_settings['row_view_history'], $lang_settings['text_minimum_class'].classlist('viewhistory',$maxclass,$AUTHORITY['viewhistory'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_VETERAN_USER,false,true,true).$lang_settings['text_view_history_note'],1);
            tr($lang_settings['row_view_topten'], $lang_settings['text_minimum_class'].classlist('topten',$maxclass,$AUTHORITY['topten'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_POWER_USER,false,true,true).$lang_settings['text_view_topten_note'],1);
            tr($lang_settings['row_view_general_log'], $lang_settings['text_minimum_class'].classlist('log',$maxclass,$AUTHORITY['log'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_INSANE_USER,false,true,true).$lang_settings['text_view_general_log_note'],1);
            tr($lang_settings['row_view_confidential_log'], $lang_settings['text_minimum_class'].classlist('confilog',$maxclass,$AUTHORITY['confilog'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_MODERATOR,false,true,true).$lang_settings['text_view_confidential_log_note'],1);
            tr($lang_settings['row_view_user_confidential'], $lang_settings['text_minimum_class'].classlist('userprofile',$maxclass,$AUTHORITY['userprofile'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_ADMINISTRATOR,false,true,true).$lang_settings['text_view_user_confidential_note'],1);
            tr($lang_settings['row_view_user_torrent'], $lang_settings['text_minimum_class'].classlist('torrenthistory',$maxclass,$AUTHORITY['torrenthistory'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_POWER_USER,false,true,true).$lang_settings['text_view_user_torrent_note'],1);
            tr($lang_settings['row_general_profile_management'],  $lang_settings['text_minimum_class'].classlist('prfmanage',$maxclass,$AUTHORITY['prfmanage'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_MODERATOR,false,true,true).$lang_settings['text_general_profile_management_note'],1);
            tr($lang_settings['row_crucial_profile_management'], $lang_settings['text_minimum_class'].classlist('cruprfmanage',$maxclass,$AUTHORITY['cruprfmanage'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_ADMINISTRATOR,false,true,true).$lang_settings['text_crucial_profile_management_note'].get_user_class_name(User::CLASS_STAFF_LEADER,false,true,true).$lang_settings['text_can_manage_donation'],1);
            tr($lang_settings['row_upload_subtitle'], $lang_settings['text_minimum_class'].classlist('uploadsub',$maxclass,$AUTHORITY['uploadsub'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_USER,false,true,true).$lang_settings['text_upload_subtitle_note'],1);
            tr($lang_settings['row_delete_own_subtitle'], $lang_settings['text_minimum_class'].classlist('delownsub',$maxclass,$AUTHORITY['delownsub'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_POWER_USER,false,true,true).$lang_settings['text_delete_own_subtitle_note'],1);
            tr($lang_settings['row_subtitle_management'], $lang_settings['text_minimum_class'].classlist('submanage',$maxclass,$AUTHORITY['submanage'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_MODERATOR,false,true,true).$lang_settings['text_subtitle_management'],1);
            tr($lang_settings['row_update_external_info'], $lang_settings['text_minimum_class'].classlist('updateextinfo',$maxclass,$AUTHORITY['updateextinfo'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_EXTREME_USER,false,true,true).$lang_settings['text_update_external_info_note'],1);
            tr($lang_settings['row_view_anonymous'], $lang_settings['text_minimum_class'].classlist('viewanonymous',$maxclass,$AUTHORITY['viewanonymous'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_UPLOADER,false,true,true).$lang_settings['text_view_anonymous_note'],1);
            tr($lang_settings['row_be_anonymous'], $lang_settings['text_minimum_class'].classlist('beanonymous',$maxclass,$AUTHORITY['beanonymous'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_CRAZY_USER,false,true,true).$lang_settings['text_be_anonymous_note'],1);
            tr($lang_settings['row_add_offer'], $lang_settings['text_minimum_class'].classlist('addoffer',$maxclass,$AUTHORITY['addoffer'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_PEASANT,false,true,true).$lang_settings['text_add_offer_note'], 1);
            tr($lang_settings['row_offer_management'], $lang_settings['text_minimum_class'].classlist('offermanage',$maxclass,$AUTHORITY['offermanage'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_MODERATOR,false,true,true).$lang_settings['text_offer_management_note'],1);
            tr($lang_settings['row_upload_torrent'], $lang_settings['text_minimum_class'].classlist('upload',$maxclass,$AUTHORITY['upload'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_POWER_USER,false,true,true).$lang_settings['text_upload_torrent_note'], 1);
            tr($lang_settings['row_upload_special_torrent'], $lang_settings['text_minimum_class'].classlist('uploadspecial',$maxclass,$AUTHORITY['uploadspecial'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_UPLOADER,false,true,true).$lang_settings['text_upload_special_torrent_note'],1);
            tr($lang_settings['row_view_special_torrent'], $lang_settings['text_minimum_class'].classlist('view_special_torrent',$maxclass,$AUTHORITY['view_special_torrent'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_CRAZY_USER,false,true,true).$lang_settings['text_view_special_torrent_note'],1);
            tr($lang_settings['row_move_torrent'], $lang_settings['text_minimum_class'].classlist('movetorrent',$maxclass,$AUTHORITY['movetorrent'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_MODERATOR,false,true,true).$lang_settings['text_move_torrent_note'],1);
            tr($lang_settings['row_chronicle_management'], $lang_settings['text_minimum_class'].classlist('chrmanage',$maxclass,$AUTHORITY['chrmanage'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_MODERATOR,false,true,true).$lang_settings['text_chronicle_management_note'],1);
            tr($lang_settings['row_view_invite'], $lang_settings['text_minimum_class'].classlist('viewinvite',$maxclass,$AUTHORITY['viewinvite'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_MODERATOR,false,true,true).$lang_settings['text_view_invite_note'],1);
            tr($lang_settings['row_buy_invites'], $lang_settings['text_minimum_class'].classlist('buyinvite',$maxclass,$AUTHORITY['buyinvite'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_INSANE_USER,false,true,true).$lang_settings['text_buy_invites_note'],1);
            tr($lang_settings['row_see_banned_torrents'], $lang_settings['text_minimum_class'].classlist('seebanned',$maxclass,$AUTHORITY['seebanned'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_UPLOADER,false,true,true).$lang_settings['text_see_banned_torrents_note'],1);
            tr($lang_settings['row_vote_against_offers'], $lang_settings['text_minimum_class'].classlist('againstoffer',$maxclass,$AUTHORITY['againstoffer'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_USER,false,true,true).$lang_settings['text_vote_against_offers_note'],1);
            tr($lang_settings['row_allow_userbar'], $lang_settings['text_minimum_class'].classlist('userbar',$maxclass,$AUTHORITY['userbar'],0,true).$lang_settings['text_default'].get_user_class_name(User::CLASS_POWER_USER,false,true,true).$lang_settings['text_allow_userbar_note'],1);
        ...
        {
            $pageTitle = $lang_settings['head_basic_settings'];
            print ($notice);
        ...
        elseif ($action == 'attachmentsettings')	// basic settings
        {
            ...
            $pageTitle = $lang_settings['head_attachment_settings'];
            print ($notice);
            print ("<form method='post' action='".$_SERVER["SCRIPT_NAME"]."'><input type='hidden' name='action' value='savesettings_attachment'>");
            $this->yesorno($lang_settings['row_enable_attachment'], 'enableattach', $ATTACHMENT["enableattach"], $lang_settings['text_enable_attachment_note']);
            tr($lang_settings['row_attachment_authority'], $lang_settings['text_attachment_authority_note_one']."<ul><li>".classlist('classone', User::CLASS_STAFF_LEADER, $ATTACHMENT['classone']) . $lang_settings['text_can_upload_at_most'] . "<input type='text' style=\"width: 50px\" name=\"countone\" value='".($ATTACHMENT['countone'] ? $ATTACHMENT['countone']: '')."'> ".$lang_settings['text_file_size_below']."<input type='text' style=\"width: 50px\" name=\"sizeone\" value='".($ATTACHMENT['sizeone'] ? $ATTACHMENT['sizeone']: '')."'>".$lang_settings['text_with_extension_name']."<input type='text' style=\"width: 200px\" name=\"extone\" value='".($ATTACHMENT['extone'] ? $ATTACHMENT['extone']: '')."'>".$lang_settings['text_authority_default_one_one'].get_user_class_name(User::CLASS_USER,false,true,true).$lang_settings['text_authority_default_one_two']."</li><li>".classlist('classtwo', User::CLASS_STAFF_LEADER, $ATTACHMENT['classtwo']) . $lang_settings['text_can_upload_at_most'] . "<input type='text' style=\"width: 50px\" name=\"counttwo\" value='".($ATTACHMENT['counttwo'] ? $ATTACHMENT['counttwo']: '')."'> ".$lang_settings['text_file_size_below']."<input type='text' style=\"width: 50px\" name=\"sizetwo\" value='".($ATTACHMENT['sizetwo'] ? $ATTACHMENT['sizetwo']: '')."'>".$lang_settings['text_with_extension_name']."<input type='text' style=\"width: 200px\" name=\"exttwo\" value='".($ATTACHMENT['exttwo'] ? $ATTACHMENT['exttwo']: '')."'>".$lang_settings['text_authority_default_two_one'].get_user_class_name(User::CLASS_POWER_USER,false,true,true).$lang_settings['text_authority_default_two_two']."</li><li>".classlist('classthree', User::CLASS_STAFF_LEADER, $ATTACHMENT['classthree']) . $lang_settings['text_can_upload_at_most'] . "<input type='text' style=\"width: 50px\" name=\"countthree\" value='".($ATTACHMENT['countthree'] ? $ATTACHMENT['countthree']: '')."'> ".$lang_settings['text_file_size_below']."<input type='text' style=\"width: 50px\" name=\"sizethree\" value='".($ATTACHMENT['sizethree'] ? $ATTACHMENT['sizethree']: '')."'>".$lang_settings['text_with_extension_name']."<input type='text' style=\"width: 200px\" name=\"extthree\" value='".($ATTACHMENT['extthree'] ? $ATTACHMENT['extthree']: '')."'>".$lang_settings['text_authority_default_three_one'].get_user_class_name(User::CLASS_VIP,false,true,true).$lang_settings['text_authority_default_three_two']."</li><li>".classlist('classfour', User::CLASS_STAFF_LEADER, $ATTACHMENT['classfour']) . $lang_settings['text_can_upload_at_most'] . "<input type='text' style=\"width: 50px\" name=\"countfour\" value='".($ATTACHMENT['countfour'] ? $ATTACHMENT['countfour']: '')."'> ".$lang_settings['text_file_size_below']."<input type='text' style=\"width: 50px\" name=\"sizefour\" value='".($ATTACHMENT['sizefour'] ? $ATTACHMENT['sizefour']: '')."'>".$lang_settings['text_with_extension_name']."<input type='text' style=\"width: 200px\" name=\"extfour\" value='".($ATTACHMENT['extfour'] ? $ATTACHMENT['extfour']: '')."'>".$lang_settings['text_authority_default_four_one'].get_user_class_name(User::CLASS_UPLOADER,false,true,true).$lang_settings['text_authority_default_four_two']."</li></ul>", 1);
        ...
        elseif ($action == 'advertisementsettings')
        {
            ...
            $pageTitle = $lang_settings['head_advertisement_settings'];
            print ($notice);
            print ("<form method='post' action='".$_SERVER["SCRIPT_NAME"]."'><input type='hidden' name='action' value='savesettings_advertisement'>");
            $this->yesorno($lang_settings['row_enable_advertisement'], 'enablead', $ADVERTISEMENT['enablead'], $lang_settings['text_enable_advertisement_note']);
            tr($lang_settings['row_no_advertisement'], "<input type='checkbox' name='enablenoad' value='yes'".($ADVERTISEMENT['enablenoad'] == 'yes' ? " checked='checked'" : "")." />".classlist('noad', User::CLASS_STAFF_LEADER, $ADVERTISEMENT['noad']).$lang_settings['text_can_choose_no_advertisement'].get_user_class_name(User::CLASS_UPLOADER,false,true,true), 1);
            tr($lang_settings['row_bonus_no_advertisement'], "<input type='checkbox' name='enablebonusnoad' value='yes'".($ADVERTISEMENT['enablebonusnoad'] == 'yes' ? " checked='checked'" : "")." />".classlist('bonusnoad', User::CLASS_STAFF_LEADER, $ADVERTISEMENT['bonusnoad']).$lang_settings['text_no_advertisement_with_bonus'].get_user_class_name(User::CLASS_POWER_USER,false,true,true), 1);
        ...
        elseif ($action == 'codesettings')	// code settings
        {
            ...
            $pageTitle = $lang_settings['head_code_settings'];
            print ($notice);
        ...
        elseif ($action == 'bonussettings'){
            ...
            $pageTitle = $lang_settings['head_bonus_settings'];
            $tagRep = new TagRepository();
            $attendance_initial_bonus = isset($BONUS['attendance_initial']) ? (int) $BONUS['attendance_initial'] : Attendance::INITIAL_BONUS;
            $attendance_step_bonus = isset($BONUS['attendance_step']) ? (int) $BONUS['attendance_step'] : Attendance::STEP_BONUS;
            $attendance_max_bonus = isset($BONUS['attendance_max']) ? (int) $BONUS['attendance_max'] : Attendance::MAX_BONUS;
            $attendance_continuous_bonus = isset($BONUS['attendance_continuous']) && is_array($BONUS['attendance_continuous']) ? $BONUS['attendance_continuous'] : Attendance::CONTINUOUS_BONUS;
            print ($notice);
            ...
            tr($lang_settings['row_buy_an_tmp_invite'],$lang_settings['text_it_costs_user']."<input type='text' style=\"width: 50px\" name=one_tmp_invite value='".(isset($BONUS["one_tmp_invite"]) ? $BONUS["one_tmp_invite"] : BonusLogs::DEFAULT_BONUS_BUY_TEMPORARY_INVITE )."'>".$lang_settings['text_buy_an_tmp_invite_note'], 1);
            ...
            $this->yesorno($lang_settings['row_allow_giving_bonus_gift'], 'bonusgift', $BONUS["bonusgift"], $lang_settings['text_giving_bonus_gift_note']);
            ...
            tr($lang_settings['row_cancel_hr'],$lang_settings['text_it_costs_user']."<input type='text' style=\"width: 50px\" name=cancel_hr value='".(isset($BONUS["cancel_hr"]) ? $BONUS["cancel_hr"] : BonusLogs::DEFAULT_BONUS_CANCEL_ONE_HIT_AND_RUN )."'>".$lang_settings['text_cancel_hr_note'], 1);
            tr($lang_settings['row_attendance_card'],$lang_settings['text_it_costs_user']."<input type='text' style=\"width: 50px\" name=attendance_card value='".(isset($BONUS["attendance_card"]) ? $BONUS["attendance_card"] : BonusLogs::DEFAULT_BONUS_BUY_ATTENDANCE_CARD )."'>".$lang_settings['text_attendance_card_note'], 1);
            tr($lang_settings['row_buy_rainbow_id'],$lang_settings['text_it_costs_user']."<input type='text' style=\"width: 50px\" name=rainbow_id value='".(isset($BONUS["rainbow_id"]) ? $BONUS["rainbow_id"] : BonusLogs::DEFAULT_BONUS_BUY_RAINBOW_ID )."'>".$lang_settings['text_buy_rainbow_id_note'], 1);
            tr($lang_settings['row_buy_change_username_card'],$lang_settings['text_it_costs_user']."<input type='text' style=\"width: 50px\" name=change_username_card value='".(isset($BONUS["change_username_card"]) ? $BONUS["change_username_card"] : BonusLogs::DEFAULT_BONUS_BUY_CHANGE_USERNAME_CARD )."'>".$lang_settings['text_buy_change_username_card_note'], 1);
            tr($lang_settings['row_self_enable'],$lang_settings['text_it_costs_user']."<input type='text' style=\"width: 50px\" name=self_enable value='".(isset($BONUS["self_enable"]) ? $BONUS["self_enable"] : BonusLogs::DEFAULT_BONUS_SELF_ENABLE )."'>".$lang_settings['text_self_enable_note'], 1);
        ...
        elseif ($action == 'accountsettings'){
            ...
            $pageTitle = $lang_settings['head_account_settings'];
            print ($notice);
            $maxclass = User::CLASS_VIP;
            ...
            tr($lang_settings['row_never_delete'],classlist('neverdelete',$maxclass,$ACCOUNT['neverdelete']).$lang_settings['text_never_delete'].get_user_class_name(User::CLASS_VETERAN_USER,false,true,true), 1);
            tr($lang_settings['row_never_delete_if_packed'],classlist('neverdeletepacked',$maxclass,$ACCOUNT['neverdeletepacked']).$lang_settings['text_never_delete_if_packed'].get_user_class_name(User::CLASS_ELITE_USER,false,true,true), 1);
            ...
            tr($lang_settings['row_ban_peasant_one'].get_user_class_name(User::CLASS_PEASANT,false,false,true).$lang_settings['row_ban_peasant_two'],get_user_class_name(User::CLASS_PEASANT,false,true,true).$lang_settings['text_ban_peasant_note_one']."<input type='text' style=\"width: 50px\" name=deletepeasant value='".(isset($ACCOUNT["deletepeasant"]) ? $ACCOUNT["deletepeasant"] : 30 )."'>".$lang_settings['text_ban_peasant_note_two'], 1);
            $inputAlias = "0_alias";
            tr($lang_settings['row_demoted_to_peasant_one'].get_user_class_name(User::CLASS_PEASANT,false,false,true).$lang_settings['row_demoted_to_peasant_two'],
                ...
                .$lang_settings['text_demoted_peasant_note_one'].get_user_class_name(User::CLASS_PEASANT,false,true,true).$lang_settings['text_demoted_peasant_note_two']."<br /><ul>
            ...
            $inputAlias = "1_alias";
            tr($lang_settings['row_default_user_one'].get_user_class_name(User::CLASS_USER,false,false,true).$lang_settings['row_default_user_two'],
            ...
            $this->promotionCriteria(User::CLASS_POWER_USER, "pu", 4, 50, 1.05, 0.95, 1, User::$classes[User::CLASS_POWER_USER]['min_seed_points'], $lang_settings, $ACCOUNT);
            $this->promotionCriteria(User::CLASS_ELITE_USER, "eu", 8, 120, 1.55, 1.45, 0, User::$classes[User::CLASS_ELITE_USER]['min_seed_points'], $lang_settings, $ACCOUNT);
            $this->promotionCriteria(User::CLASS_CRAZY_USER, "cu", 15, 300, 2.05, 1.95, 2, User::$classes[User::CLASS_CRAZY_USER]['min_seed_points'], $lang_settings, $ACCOUNT);
            $this->promotionCriteria(User::CLASS_INSANE_USER, "iu", 25, 500, 2.55, 2.45, 0, User::$classes[User::CLASS_INSANE_USER]['min_seed_points'], $lang_settings, $ACCOUNT);
            $this->promotionCriteria(User::CLASS_VETERAN_USER, "vu", 40, 750, 3.05, 2.95, 3, User::$classes[User::CLASS_VETERAN_USER]['min_seed_points'], $lang_settings, $ACCOUNT);
            $this->promotionCriteria(User::CLASS_EXTREME_USER, "exu", 60, 1024, 3.55, 3.45, 0, User::$classes[User::CLASS_EXTREME_USER]['min_seed_points'], $lang_settings, $ACCOUNT);
            $this->promotionCriteria(User::CLASS_ULTIMATE_USER, "uu", 80, 1536, 4.05, 3.95, 5, User::$classes[User::CLASS_ULTIMATE_USER]['min_seed_points'], $lang_settings, $ACCOUNT);
            $this->promotionCriteria(User::CLASS_NEXUS_MASTER, "nm", 100, 3072, 4.55, 4.45, 10, User::$classes[User::CLASS_NEXUS_MASTER]['min_seed_points'], $lang_settings, $ACCOUNT);
            tr($lang_settings['row_save_settings'],"<input type='submit' name='save' value='".$lang_settings['submit_save_settings']."'>", 1);
            print ("</form>");
        }
        elseif ($action == 'torrentsettings')
        {
            ...
            $pageTitle = $lang_settings['head_torrent_settings'];
            print ($notice);
            ...
            $this->yesorno($lang_settings['row_download_support_passkey'], 'download_support_passkey', $TORRENT["download_support_passkey"] ?? '', $lang_settings['text_download_support_passkey_note']);
            $this->yesorno($lang_settings['row_approval_status_icon_enabled'], 'approval_status_icon_enabled', $TORRENT["approval_status_icon_enabled"] ?? '', $lang_settings['text_approval_status_icon_enabled_note']);
            $this->yesorno($lang_settings['row_approval_status_none_visible'], 'approval_status_none_visible', $TORRENT["approval_status_none_visible"] ?? '', $lang_settings['text_approval_status_none_visible_note']);

            $nfoViewStyleRadio = '';
            $name = 'nfo_view_style_default';
            foreach (Torrent::$nfoViewStyles as $style => $info) {
            ...
            $this->yesorno($lang_settings['row_paid_torrent_enabled'], 'paid_torrent_enabled', $TORRENT["paid_torrent_enabled"] ?? '', $lang_settings['text_paid_torrent_enabled_note']);
...
<li>".sprintf($lang_settings['claim_torrent_ttl'], sprintf('<input type="number" name="claim_torrent_ttl" value="%s" style="width: 50px"/>', $TORRENT['claim_torrent_ttl'] ?? Claim::TORRENT_TTL))."</li>
<li>".sprintf($lang_settings['claim_torrent_user_counts_up_limit'], sprintf('<input type="number" name="claim_torrent_user_counts_up_limit" value="%s" style="width: 50px"/>', $TORRENT['claim_torrent_user_counts_up_limit'] ?? Claim::USER_UP_LIMIT))."</li>
<li>".sprintf($lang_settings['claim_user_torrent_counts_up_limit'], sprintf('<input type="number" name="claim_user_torrent_counts_up_limit" value="%s" style="width: 50px"/>', $TORRENT['claim_user_torrent_counts_up_limit'] ?? Claim::TORRENT_UP_LIMIT))."</li>
<li>".sprintf($lang_settings['claim_remove_deduct_user_bonus'], sprintf('<input type="number" name="claim_remove_deduct_user_bonus" value="%s" style="width: 50px"/>', $TORRENT['claim_remove_deduct_user_bonus'] ?? Claim::REMOVE_DEDUCT))."</li>
<li>".sprintf($lang_settings['claim_give_up_deduct_user_bonus'], sprintf('<input type="number" name="claim_give_up_deduct_user_bonus" value="%s" style="width: 50px"/>', $TORRENT['claim_give_up_deduct_user_bonus'] ?? Claim::GIVE_UP_DEDUCT))."</li>
<li>".sprintf($lang_settings['claim_bonus_multiplier'], sprintf('<input type="number" name="claim_bonus_multiplier" value="%s" style="width: 50px"/>', $TORRENT['claim_bonus_multiplier'] ?? Claim::BONUS_MULTIPLIER))."</li>
<li>".sprintf($lang_settings['claim_reach_standard'], sprintf('<input type="number" name="claim_reach_standard_seed_time" value="%s" style="width: 50px"/>', $TORRENT['claim_reach_standard_seed_time'] ?? Claim::STANDARD_SEED_TIME_HOURS), sprintf('<input type="number" name="claim_reach_standard_uploaded" value="%s" style="width: 50px"/>', $TORRENT['claim_reach_standard_uploaded'] ?? Claim::STANDARD_UPLOADED_TIMES))."</li>
</ul>", 1);
        ...
        elseif ($action == 'mainsettings')	// main settings
        {
            ...
            $pageTitle = $lang_settings['head_main_settings'];
            print ($notice);
            print ("<form method='post' action='".$_SERVER["SCRIPT_NAME"]."'><input type='hidden' name='action' value='savesettings_main'>");
            $sh = "gmail.com";

            $this->yesorno($lang_settings['row_site_online'], 'site_online', $MAIN['site_online'], $lang_settings['text_site_online_note']);
            $this->yesorno($lang_settings['row_enable_invite_system'], 'invitesystem', $MAIN['invitesystem'], $lang_settings['text_invite_system_note']);
            ...
            $this->yesorno($lang_settings['row_complain_enabled'], 'complain_enabled', $MAIN['complain_enabled'], $lang_settings['row_complain_enabled_note']);
            $this->yesorno($lang_settings['row_enable_registration_system'], 'registration', $MAIN['registration'], $lang_settings['row_allow_registrations']);
            ...
            $this->yesorno($lang_settings['row_enable_wait_system'],'waitsystem', $MAIN['waitsystem'], $lang_settings['text_wait_system_note']);
            $this->yesorno($lang_settings['row_enable_max_slots_system'],'maxdlsystem', $MAIN['maxdlsystem'], $lang_settings['text_max_slots_system_note']);
            $this->yesorno($lang_settings['row_show_polls'], 'showpolls', $MAIN['showpolls'], $lang_settings['text_show_polls_note']);
            $this->yesorno($lang_settings['row_show_stats'],'showstats', $MAIN['showstats'], $lang_settings['text_show_stats_note']);
            $this->yesorno($lang_settings['row_show_last_posts'],'showlastxforumposts', $MAIN['showlastxforumposts'], $lang_settings['text_show_last_posts_note']);
            $this->yesorno($lang_settings['row_show_last_torrents'],'showlastxtorrents', $MAIN['showlastxtorrents'], $lang_settings['text_show_last_torrents_note']);
            $this->yesorno($lang_settings['row_show_server_load'],'showtrackerload', $MAIN['showtrackerload'], $lang_settings['text_show_server_load_note']);
            $this->yesorno($lang_settings['row_show_forum_stats'],'showforumstats', $MAIN['showforumstats'], $lang_settings['text_show_forum_stats_note']);
            $this->yesorno($lang_settings['row_show_hot'],'showhotmovies', $MAIN['showhotmovies'], $lang_settings['text_show_hot_note']);
            $this->yesorno($lang_settings['row_show_classic'],'showclassicmovies', $MAIN['showclassicmovies'], $lang_settings['text_show_classic_note']);
            $this->yesorno($lang_settings['row_show_top_uploader'],'show_top_uploader', $MAIN['show_top_uploader'], $lang_settings['text_show_top_uploader_note']);
            $this->yesorno($lang_settings['row_enable_imdb_system'],'showimdbinfo', $MAIN['showimdbinfo'], $lang_settings['text_imdb_system_note']);

            $imdbLanguages = "<select name=imdb_language>";
            foreach (Imdb::listSupportLanguages() as $key => $value) {
            ...
            $this->yesorno($lang_settings['row_enable_pt_gen_system'],'enable_pt_gen_system', $MAIN['enable_pt_gen_system'], $lang_settings['text_enable_pt_gen_system_note']);
            ...
            $this->yesorno($lang_settings['row_enable_nfo'],'enablenfo', $MAIN['enablenfo'], $lang_settings['text_enable_nfo_note']);
            $this->yesorno($lang_settings['row_enable_technical_info'],'enable_technical_info', $MAIN['enable_technical_info'], $lang_settings['text_enable_technical_info']);
            $this->yesorno($lang_settings['row_enable_school_system'],'enableschool', $MAIN['enableschool'], $lang_settings['text_school_system_note']);
            $this->yesorno($lang_settings['row_enable_global_search_system'],'enable_global_search', $MAIN['enable_global_search'], $lang_settings['text_global_search_system_note']);
            $this->yesorno($lang_settings['row_restrict_email_domain'],'restrictemail', $MAIN['restrictemail'], $lang_settings['text_restrict_email_domain_note']);
            $this->yesorno($lang_settings['row_show_shoutbox'],'showshoutbox', $MAIN['showshoutbox'], $lang_settings['text_show_shoutbox_note']);
            $this->yesorno($lang_settings['row_show_funbox'],'showfunbox', $MAIN['showfunbox'], $lang_settings['text_show_funbox_note']);
            $this->yesorno($lang_settings['row_enable_offer_section'],'showoffer', $MAIN['showoffer'], $lang_settings['text_offer_section_note']);
            $this->yesorno($lang_settings['row_show_donation'],'donation', $MAIN['donation'], $lang_settings['text_show_donation_note']);
            $this->yesorno($lang_settings['row_weekend_free_uploading'],'sptime', $MAIN['sptime'], $lang_settings['text_weekend_free_uploading_note']);
            $this->yesorno($lang_settings['row_enable_helpbox'],'showhelpbox', $MAIN['showhelpbox'], $lang_settings['text_helpbox_note']);
            $this->yesorno($lang_settings['row_enable_bitbucket'],'enablebitbucket', $MAIN['enablebitbucket'], $lang_settings['text_bitbucket_note']);
            $this->yesorno($lang_settings['row_enable_small_description'],'smalldescription', $MAIN['smalldescription'], $lang_settings['text_small_description_note']);
            $this->yesorno($lang_settings['row_ptshow_naming_style'],'altname', $MAIN['altname'], $lang_settings['text_ptshow_naming_style_note']);
            $this->yesorno($lang_settings['row_use_external_forum'],'extforum', $MAIN['extforum'], $lang_settings['text_use_external_forum_note']);
            ...
            $this->yesorno($lang_settings['row_show_special_section'],'spsct', $MAIN['spsct'], $lang_settings['text_show_special_section_note']);
            $bcatlist = $scatlist = '';
            foreach (SearchBox::query()->select(['id', 'name'])->get() as $searchbox) {
                $bcatlist .= "<input type=radio name=browsecat value='".$searchbox->id."'".($MAIN["browsecat"] == $searchbox->id ? " checked" : "").">".$searchbox->name."&nbsp;";
                $scatlist .= "<input type=radio name=specialcat value='".$searchbox->id."'".($MAIN["specialcat"] == $searchbox->id ? " checked" : "").">".$searchbox->name."&nbsp;";
            }
            ...
            $allEnabled = Language::listEnabled(true);
            $langlist = "";
...
JS;
            ...
            foreach (DB::table('stylesheets')->orderBy('name')->get() as $ss) {
                $csslist .= "<option value='".$ss->id."'".($MAIN["defstylesheet"] == $ss->id ? " selected" : "").">".$ss->name."</option>";
            }
            $csslist .= "</select>";
        ...
        {
            $result = Setting::getByWhereRaw("name like 'misc.%'");
            $misc = $result['misc'] ?? [];
            $pageTitle = $lang_settings['row_misc_settings'];
            print ($notice);
        ...
        {
            $pageTitle = $lang_settings['head_website_settings'];
            print ($notice);
        ...
        return view('settings', compact('content') + ['pageTitle' => $pageTitle]);
    }
    ...
    private function yesorno($title, $name, $value, $note="")
    {
        global $lang_settings;
    ...
    private function promotionCriteria($class, $input, $time, $dl, $prratio, $deratio, $defaultInvites, $defaultSeedPoints, array $lang_settings, array $ACCOUNT)
    {
        ...
        $y = $lang_settings['text_alias'] . "<input type='text' style=\"width: 60px\" name='".$inputAlias."' value='".(isset($ACCOUNT[$inputAlias]) ? $ACCOUNT[$inputAlias] : '' )."'><br/>"
